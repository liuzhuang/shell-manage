...
1. 访问 [ShellManage 官网](https://shell-manage.vercel.app)，从 GitHub Releases 下载与系统和芯片架构匹配的安装包。
...
- macOS Intel：`ShellManage-<version>-macos-x64.dmg`
- macOS Apple Silicon：`ShellManage-<version>-macos-arm64.dmg`

当前产物是未使用平台证书的预览版，不需要 Apple Developer 或 Windows 代码签名账号。Windows 使用 GitHub Releases 自动下载与安装更新；macOS 应用使用 ad-hoc 签名，更新文件使用 Sparkle EdDSA 签名，点击后自动下载、替换应用并重启。由于安装包没有 Developer ID 签名和公证，系统仍可能显示 Gatekeeper、「未知发布者」或 SmartScreen 提示。
...
开发环境与官网部署见 [开发与部署](DEVELOPMENT.md)，版本发布操作见 [发版说明](RELEASING.md)。
